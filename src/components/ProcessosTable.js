...
const getDeadlinePillClass = (isoString) => {
  ...
  try {
    ...
    const base = 'px-2 py-0.5 rounded text-xs font-semibold inline-flex items-center';

    if (diffTime <= 0) return `${base} bg-red-100 text-red-700 border border-red-300`;
...
const getPriorityBadgeClasses = (status) => {
  ...
  switch (color) {
    case 'red':    return `${base} bg-red-50 text-red-800 border-red-200`;
    case 'orange': return `${base} bg-orange-50 text-orange-800 border-orange-200`;
    case 'blue':   return `${base} bg-blue-50 text-blue-800 border-blue-200`;
    case 'purple': return `${base} bg-purple-50 text-purple-800 border-purple-200`;
    case 'amber':  return `${base} bg-amber-50 text-amber-800 border-amber-200`;
    case 'green':  return `${base} bg-green-50 text-green-800 border-green-200`;
    default:       return `${base} bg-[#EDE8E5] text-[#161616]/70 border-[#AA8F71]/30`;
  }
};

const ProcessosTable = ({ processos, onProcessoClick }) => {
  return (
    <div className="bg-white rounded-xl overflow-hidden border border-[#AA8F71]/20 shadow-sm">
            ...
            processos.map((processo, i) => (
              ...
              >
                ...
                <td className="px-5 py-3.5">
                  <span className={getPriorityBadgeClasses(processo.statusPrioridade)}>
                ...
                <td className="px-5 py-3.5 text-right">
                  <button
                    onClick={(e) => { e.stopPropagation(); onProcessoClick(processo); }}
